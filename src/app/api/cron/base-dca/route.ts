...
import { runEvmDcaForAllUsers } from "@/lib/evm/dca";
 ...
 *
 * runEvmDcaForAllUsers() also checks each user's monthly auto-sweep (send
 * WETH above a configured minimum to a cold wallet) on every run — it has
...
export async function GET(req: NextRequest) {
    ...
    const results = await runEvmDcaForAllUsers();
