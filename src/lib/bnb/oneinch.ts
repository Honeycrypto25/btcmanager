...
function sdk(): Sdk {
    return new Sdk({ authKey: apiKey(), networkId: BNB_CHAIN_ID, httpConnector: new FetchProviderConnector() });
}
...
/** Derived from a real swap quote (1 WBNB -> USDT) rather than a separate spot-price endpoint, so it reflects what the bot would actually get. */
export async function getBnbPriceUsd(): Promise<number> {
    const oneWeth = parseUnits("1", 18).toString();
    const qs = new URLSearchParams({ src: WBNB_ADDRESS, dst: USDT_ADDRESS, amount: oneWeth });
    const res = await oneinchFetch<QuoteResponse>(`${ONEINCH_SWAP_API_BASE}/${BNB_CHAIN_ID}/quote?${qs.toString()}`);
    return Number(formatUnits(res.dstAmount, USDT_DECIMALS));
}

/** Builds, signs (by sending the returned tx) and confirms a swap. Returns the tx hash, gas paid (in BNB), and the destination amount received. */
export async function executeSwap(wallet: Wallet, params: {
    srcToken: string;
    dstToken: string;
    amountRaw: bigint;
    slippageBps: number;
}): Promise<{ txHash: string; feeBnb: number; dstAmountRaw: string }> {
    ...
    const quote = await oneinchFetch<SwapResponse>(`${ONEINCH_SWAP_API_BASE}/${BNB_CHAIN_ID}/swap?${qs.toString()}`);
    ...
    const feeBnb = Number(formatUnits(receipt.gasUsed * receipt.gasPrice, 18));
    return { txHash: sent.hash, feeBnb, dstAmountRaw: quote.dstAmount };
}

// --- Limit Order Protocol v4 (used for the take-profit sell order) ---

/** Signs (gasless, off-chain) and submits a fill-or-kill limit order selling `makingAmountRaw` WBNB for `takingAmountRaw` USDT once a resolver takes it. */
export async function createTakeProfitSellOrder(wallet: Wallet, params: {
    ...
    await ensureAllowance(wallet, WBNB_ADDRESS, getLimitOrderContract(BNB_CHAIN_ID), params.makingAmountRaw);
    ...
    const order = await client.createOrder(
        {
            makerAsset: new OneInchAddress(WBNB_ADDRESS),
            takerAsset: new OneInchAddress(USDT_ADDRESS),
    ...
    const typedData = order.getTypedData(BNB_CHAIN_ID);
    ...
    return { orderHash: order.getOrderHash(BNB_CHAIN_ID) };
}

function api(): Api {
    return new Api({ authKey: apiKey(), networkId: BNB_CHAIN_ID, httpConnector: new FetchProviderConnector() });
}
