 ...
 * day the interval isn't due for a fresh buy. Mirrors the BNB side exactly.
 */
async function retryPendingSellOrders(
    ...
    const stuckLots = await db.evmLot.findMany({ where: { userId, status: "PENDING_SELL_ORDER" } });
