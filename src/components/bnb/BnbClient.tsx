...
import { upsertBnbSettings, runBnbDcaNow, runBnbSweepNow, type BnbSettingsInput } from "@/app/actions/bnb";
import { formatUsd, type SettingsDTO } from "./shared";
import { useIsAdmin } from "@/hooks/useIsAdmin";

/** Vercel Cron for /api/cron/bnb-dca runs daily at 09:20 UTC (see vercel.json) — this just mirrors that schedule for display. */
function nextCronRunUtc(): Date {
    const now = new Date();
    const next = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), 9, 20, 0));
    if (next.getTime() <= now.getTime()) next.setUTCDate(next.getUTCDate() + 1);
    return next;
}

export function BnbClient({
    initialSettings,
    bnbPriceUsd,
    botWallet,
    sweepDestination,
    ...
    const [form, setForm] = useState<BnbSettingsInput>({
        ...
        sweepMinBalanceBnb: 0, // hardcoded — WBNB pays no fees on BNB Chain, so there is no reason to keep a minimum
    });
    ...
    function handleSave() {
        ...
        startSaving(async () => {
            try {
                const saved = await upsertBnbSettings(form);
    ...
    function confirmSweepNow() {
        ...
        startSweeping(async () => {
            try {
                const result = await runBnbSweepNow();
                if (result.action === "sent") setSweepMessage(`Trimis ${result.amountBnb} WBNB — pagina se va reîmprospăta.`);
    ...
    function handleRunNow() {
        ...
        startRunning(async () => {
            try {
                const result = await runBnbDcaNow();
        ...
        <div className="space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="font-display text-xl font-medium text-foreground">BNB Chain — DCA automat</h1>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-faint">
                                {settings
                                    ? `${settings.walletAddress.slice(0, 6)}...${settings.walletAddress.slice(-4)} · $${settings.buyAmountUsd}/${settings.intervalHours}h · țintă +${settings.takeProfitPercent}%`
                                    : "Nesalvat încă"}
                            </p>
                        </div>
                    </div>
            ...
            <Card className="space-y-4">
                ...
                {"error" in botWallet ? (
                    <div className="flex items-start gap-2 rounded-lg border border-amber-400/30 bg-amber-500/10 px-3 py-2.5 text-sm text-amber-200">
                        ...
                        <span>
                            <code>BASE_PRIVATE_KEY</code> nu e configurată încă în Vercel — setează-o, apoi revino aici. ({botWallet.error})
                        </span>
                    </div>
                ) : (
                ...
                <p className="text-xs text-faint">
                    Portofelul trebuie să aibă în prealabil USDT (pentru cumpărare) și puțin BNB (pentru gas — spre deosebire de Solana, gas-ul e plătit dintr-un token separat de WBNB-ul acumulat).
            ...
            <Card className="space-y-4">
                ...
                <p className="text-xs text-faint">
                    În fiecare zi de 2 a lunii, tot WBNB-ul disponibil se trimite automat către portofelul de retragere — floatul de BNB pentru gas nu e niciodată atins (WBNB nu plătește fees pe BNB Chain), și nici WBNB-ul deja promis unui ordin de vânzare activ (spre deosebire de Solana, un ordin 1inch nu blochează fondurile on-chain, deci scădem manual ce e rezervat înainte de a calcula excedentul). Suma trimisă e rotunjită în jos la 6 zecimale.
                </p>
                {settings && !editing && (
                    <p className="text-xs text-faint">
                ...
                {"error" in sweepDestination ? (
                    <div className="flex items-start gap-2 rounded-lg border border-amber-400/30 bg-amber-500/10 px-3 py-2.5 text-sm text-amber-200">
                        <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />
                        <span>
                            <code>BNB_SWEEP_DESTINATION</code> nu e configurată încă în Vercel — setează-o, apoi revino aici. Retragerea rămâne dezactivată până atunci, indiferent de comutatorul de mai sus.
                        </span>
                    </div>
                ) : (
                    <div className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2.5 text-sm">
                    ...
                    <Card className="max-w-md w-full space-y-5 border-amber-400/30">
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-xs text-faint">
                                    Se trimite o tranzacție reală și ireversibilă pe blockchain-ul BNB Chain.
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-2 rounded-lg border border-white/10 bg-white/[0.03] px-3 py-3 text-sm">
                            <div className="flex items-center gap-2 text-faint">
                                ...
                                <span className="text-xs">Tot WBNB-ul disponibil (mai puțin ce e rezervat pentru ordine deschise), rotunjit în jos la 6 zecimale</span>
                            </div>
