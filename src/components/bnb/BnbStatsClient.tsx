...
export function BnbStatsClient({
    lots: allLots,
    bnbPriceUsd,
    sweeps,
    fuelStatus,
    ...
    sweeps: SweepDTO[];
    fuelStatus: BnbFuelStatus | { error: string };
}) {
    ...
    async function handleCheckNow() {
        ...
        setCheckError(null);
        try {
            const result = await reconcileBnbOrdersNow();
    ...
    const stats = useMemo(() => {
        ...
        return { totalInvestedUsd, totalRealizedProceedsUsd, totalRealizedPnlUsd, totalFeesUsd, bnbHeld, openOrders };
    }, [lots]);
    ...
    const monthlyData = useMemo(() => {
        ...
        for (const lot of allLots) {
            ...
            byMonth.set(key, (byMonth.get(key) ?? 0) + Number(lot.bnbRemaining));
        }
    ...
    // WBNB actually sent out to the sweep destination — only SUCCESS rows count.
    const monthlySweepData = useMemo(() => {
        ...
        for (const sweep of sweeps) {
            ...
            byMonth.set(key, (byMonth.get(key) ?? 0) + Number(sweep.amountBnb));
        }
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h1 className="font-display text-xl font-medium text-foreground">BNB Chain — Statistici</h1>
                    <p className="text-sm text-muted">
                        {bnbPriceUsd ? `Preț curent: ${formatUsd(bnbPriceUsd)}` : "Preț curent indisponibil momentan"}
                    </p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
                ...
                <StatCard label="WBNB deținut" value={`${stats.bnbHeld.toFixed(5)} WBNB`} />
                <StatCard label="Ordine active" value={String(stats.openOrders)} />
            </div>

            {/* Fuel — how many more buy cycles the current USDT balance can fund */}
            <Card>
                <h2 className="mb-1 text-sm font-medium text-foreground">Combustibil (USDT)</h2>
                {"error" in fuelStatus ? (
                    <p className="text-sm text-muted">{fuelStatus.error}</p>
                ) : fuelStatus.usdtBalance <= 0 ? (
                    <p className="text-sm text-muted">
                        Portofelul botului are 0 USDT — alimentează-l ca DCA-ul să poată continua.
                    </p>
                ) : (
                    <>
                        <p className="mb-4 text-xs text-faint">
                            {formatUsd(fuelStatus.usdtBalance)} USDT în portofel ÷ {formatUsd(fuelStatus.buyAmountUsd)} pe ciclu, la fiecare {fuelStatus.intervalHours}h ≈{" "}
                        ...
                        <ResponsiveContainer width="100%" height={220}>
                            <BarChart data={fuelStatus.projected}>
                                ...
                                />
                                <Bar dataKey="usdt" name="USDT proiectat" fill="rgba(96,165,250,0.7)" radius={[4, 4, 0, 0]} />
                            </BarChart>
                        </ResponsiveContainer>
                    </>
                )}
                        ...
                        <ComposedChart data={chartData}>
                            ...
                            {bnbPriceUsd && (
                                <ReferenceLine
                                    ...
                                    stroke="#e5e7eb"
                                    strokeDasharray="2 3"
                                    label={{ value: `Preț curent: ${formatUsd(bnbPriceUsd)}`, position: "insideTopRight", fill: "#e5e7eb", fontSize: 11 }}
                                />
                            )}
            ...
            {monthlyData.length > 0 && (
                <Card>
                    <h2 className="mb-1 text-sm font-medium text-foreground">WBNB acumulat pe lună</h2>
                    <p className="mb-4 text-xs text-faint">
                        Ultimele 14 luni, doar cicluri finalizate — WBNB rămas efectiv în portofel după vânzare (profitul), nu suma cumpărată. Loturile în așteptare nu sunt incluse încă.
                    </p>
                    <ResponsiveContainer width="100%" height={240}>
                        <BarChart data={monthlyData}>
                            ...
                            <Tooltip
                                contentStyle={{ background: "#111", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, fontSize: 12 }}
                                formatter={(v) => `${Number(v).toFixed(5)} WBNB`}
                            />
                            <Bar dataKey="weth" name="WBNB cumpărat" fill="rgba(98,126,234,0.7)" radius={[4, 4, 0, 0]} />
                        </BarChart>
                    </ResponsiveContainer>
                </Card>
            )}
            ...
            <Card>
                ...
                <p className="mb-4 text-xs text-faint">
                    Ultimele 14 luni, doar retrageri reușite — WBNB trimis efectiv peste minimul păstrat în portofelul botului.
                </p>
                <ResponsiveContainer width="100%" height={240}>
                    <BarChart data={monthlySweepData}>
                        ...
                        <Tooltip
                            ...
                            formatter={(v) => `${Number(v).toFixed(5)} WBNB`}
                        />
                        <Bar dataKey="weth" name="WBNB retras" fill="rgba(96,165,250,0.7)" radius={[4, 4, 0, 0]} />
                    </BarChart>
                </ResponsiveContainer>
            </Card>
...
function BscscanLink({ hash, label }: { hash: string; label: string }) {
    return (
        <a
            ...
            <table className="w-full min-w-[1180px] text-left text-sm">
                <thead>
                    <tr className="text-xs uppercase tracking-wider text-faint">
                        ...
                        <th className="pb-2 pr-4">WBNB primit</th>
                    ...
                    {pageItems.map((lot) => {
                        ...
                        return (
                            <tr key={lot.id} className="border-t border-white/[0.06]">
                                ...
                                <td className="py-2 pr-4 text-foreground">{Number(lot.bnbAcquired).toFixed(5)}</td>
                                ...
                                <td className="py-2 pr-4 text-foreground">{Number(lot.bnbRemaining).toFixed(5)}</td>
                ...
                <tbody>
                    {pageItems.map((sweep) => (
                        <tr key={sweep.id} className="border-t border-white/[0.06]">
                            ...
                            <td className="py-2 pr-4 text-foreground">{Number(sweep.balanceBeforeBnb).toFixed(5)} WBNB</td>
                            <td className="py-2 pr-4 text-foreground">{Number(sweep.amountBnb).toFixed(5)} WBNB</td>
                            ...
                            <td className="py-2 text-faint">
                                {sweep.txHash ? <BscscanLink hash={sweep.txHash} label="Vezi ↗" /> : "—"}
                            </td>
                        </tr>
                    ))}
                </tbody>
