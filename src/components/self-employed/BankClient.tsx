...
function TransactionsTab({ transactions, accounts, matchableReceipts }: { transactions: TransactionRow[]; accounts: Account[]; matchableReceipts: MatchableReceipt[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (showUnlabeledOnly) list = list.filter((r) => !r.convertedType);
    ...
    return list;
  }, [rows, filter, showUnlabeledOnly, dateFrom, dateTo, accountFilter, search]);

  const hasActiveFilters = !!(filter || showUnlabeledOnly || dateFrom || dateTo || accountFilter || search.trim());
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [filter, showUnlabeledOnly, dateFrom, dateTo, accountFilter, search]);
  ...
  const counts = {
    ...
    unlabeled: rows.filter((r) => !r.convertedType).length,
  };
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex flex-wrap gap-2">
          ...
          <button
            onClick={() => setShowUnlabeledOnly((v) => !v)}
            className={cn(
              "rounded-lg px-3 py-1.5 text-xs font-medium transition-colors border",
              showUnlabeledOnly ? "bg-amber-500 text-black border-amber-500" : "bg-glass border-amber-400/30 text-amber-300 hover:text-amber-200"
            )}
            title="Tranzacții care nu sunt marcate nici ca venit, nici ca cheltuială, nici ca ignorate"
          >
            Neetichetate ({counts.unlabeled})
          </button>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-border bg-white/[0.02]">
                ...
                <th className="px-6 py-4 text-[10px] text-muted uppercase text-xs font-medium tracking-wider">Etichetă</th>
              ...
              {filtered.length === 0 ? (
                ...
                paginated.map((tx) => (
                  <React.Fragment key={tx.id}>
                    <tr className={cn("hover:bg-white/[0.01] transition-colors", !tx.convertedType && "bg-amber-500/[0.04] border-l-2 border-l-amber-400/50")}>
                      ...
                      <td className="px-6 py-4">
                        {tx.convertedType ? (
                          ...
                          <span className="inline-flex items-center gap-1 rounded-full border border-amber-400/30 bg-amber-500/10 px-2.5 py-1 text-[10px] font-medium uppercase tracking-wider text-amber-300">
