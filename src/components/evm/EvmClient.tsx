...
function nextCronRunUtc(): Date {
    const now = new Date();
    const next = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), 9, 10, 0));
...
export function EvmClient({
    ...
    function handleSave() {
        ...
        startSaving(async () => {
            try {
                const saved = await upsertEvmSettings(form);
    ...
    function confirmSweepNow() {
        ...
        startSweeping(async () => {
            try {
                const result = await runEvmSweepNow();
                if (result.action === "sent") setSweepMessage(`Trimis ${result.amountWeth} WETH — pagina se va reîmprospăta.`);
    ...
    function handleRunNow() {
        ...
        startRunning(async () => {
            try {
                const result = await runEvmDcaNow();
        ...
        <div className="space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="font-display text-xl font-medium text-foreground">Base (ETH) — DCA automat</h1>
            ...
            <Card className="space-y-4">
                ...
                {"error" in sweepDestination ? (
                    <div className="flex items-start gap-2 rounded-lg border border-amber-400/30 bg-amber-500/10 px-3 py-2.5 text-sm text-amber-200">
                        <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />
                        <span>
                            <code>BASE_SWEEP_DESTINATION</code> nu e configurată încă în Vercel — setează-o, apoi revino aici. Retragerea rămâne dezactivată până atunci, indiferent de comutatorul de mai sus.
                        </span>
                    </div>
                ) : (
                    <div className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2.5 text-sm">
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-xs text-faint">
                                    Se trimite o tranzacție reală și ireversibilă pe blockchain-ul Base.
                                </p>
                            </div>
                        </div>
