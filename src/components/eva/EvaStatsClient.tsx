...
export function EvaStatsClient({
    lots: allLots,
    evaPriceUsd,
    sweeps,
    fuelStatus,
    ...
    async function handleCheckNow() {
        ...
        setCheckError(null);
        try {
            const result = await reconcileEvaOrdersNow();
    ...
    const stats = useMemo(() => {
        ...
        return { totalInvestedUsd, totalRealizedProceedsUsd, totalRealizedPnlUsd, totalFeesUsd, evaHeld, openOrders };
    }, [lots]);
    ...
    // lot's sell hasn't happened yet, so how much EVA ultimately stays
    ...
    const monthlyData = useMemo(() => {
        ...
        for (const lot of allLots) {
            ...
            byMonth.set(key, (byMonth.get(key) ?? 0) + Number(lot.evaRemaining));
        }

        return months.map(({ key, label }) => ({
            month: label,
            eva: Math.round((byMonth.get(key) ?? 0) * 10000) / 10000,
        }));
    }, [allLots]);
    ...
    const durationByPercentData = useMemo(() => {
        ...
        for (const lot of allLots) {
            ...
            const percent = Math.round((targetPrice / buyPrice - 1) * 1000) / 10; // one decimal, e.g. 11.1
            const key = `${percent}%`;
    ...
    // EVA actually sent out to the sweep destination — only SUCCESS rows
    ...
    const monthlySweepData = useMemo(() => {
        ...
        for (const sweep of sweeps) {
            ...
            byMonth.set(key, (byMonth.get(key) ?? 0) + Number(sweep.amountEva));
        }
    ...
    return (
        <div className="space-y-6">
            ...
            {monthlyData.length > 0 && (
                <Card>
                    <h2 className="mb-1 text-sm font-medium text-foreground">EVA acumulat pe lună</h2>
                    <p className="mb-4 text-xs text-faint">
                        Ultimele 14 luni, doar cicluri finalizate — EVA rămas efectiv în portofel după vânzare (profitul), nu suma cumpărată. Loturile în așteptare nu sunt incluse încă, fiindcă nu se știe cât EVA rămâne până la finalizarea vânzării.
                    </p>
                    <ResponsiveContainer width="100%" height={240}>
            ...
            <Card>
                ...
                <p className="mb-4 text-xs text-faint">
                    Ultimele 14 luni, doar retrageri reușite — EVA trimis efectiv peste minimul păstrat în portofelul botului.
                </p>
                <ResponsiveContainer width="100%" height={240}>
            ...
            {/* One row per cycle — buy leg (always a done, confirmed transaction the
               moment a lot exists) shown together with wherever its sell side currently
               stands, instead of split across separate tables. */}
